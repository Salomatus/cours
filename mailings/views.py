...
from .services import Get_mailings_statistics, send_mailing
...
def send_mailing(request):
    if request.method == "POST":
        ...
        form = MailingForm()
    return render(request, "mailings/send_mailing.html", {"form": form})
...
class MailingListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.groups.filter(name="Managers").exists():
            statistics = Get_mailings_statistics()
        else:
            statistics = Get_mailings_statistics(self.request.user)
